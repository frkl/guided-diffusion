...
class new(nn.Module):
    ...
    def forward(self,x0):
        ...
        eps=x0.data.clone().normal_()
        T=torch.LongTensor(x0.shape[0]).random_(0,self.T)
        
        w0=self.alpha_bar[T]**0.5
        w1=(1-self.alpha_bar[T])**0.5
        x=w0.to(device).view(-1,1,1,1)*x0+w1.to(device).view(-1,1,1,1)*eps
        x=x.float()
        
        v=w0.to(device).view(-1,1,1,1)*eps-w1.to(device).view(-1,1,1,1)*x0
        
        v_pred = self.encoder(x, T.to(v.device))
        loss=((v-v_pred)**2).sum(dim=-3).mean()
