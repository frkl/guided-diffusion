...
def default_params():
    ...
    params.data='/work2/dataset/turbo-fgvc-1'
...
net = Unet(dim = params.nh,dim_mults = (1, 2, 4, 8),flash_attn = True).cuda()
diffusion = GaussianDiffusion(net,image_size = params.imsz,beta_schedule='cosine',objective='pred_noise',timesteps = 1000).cuda()
    ...
    if (epoch+0)%10==0:
        ...
        with torch.no_grad():
            ...
            for i,v in enumerate(data_test):
                ...
                if i==0:
                    gen = diffusion.sample(batch_size = 16)
                    print(gen.shape,gen.max(),gen.min())
                    util.video.write_video(gen,'debug_diffusion.avi',fps=3)
